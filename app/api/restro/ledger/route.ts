...
function indiaDateStartToUtcIso(value: string) {
  return new Date(`${value}T00:00:00+05:30`).toISOString();
}

function indiaDateEndToUtcIso(value: string) {
  return new Date(`${value}T23:59:59.999+05:30`).toISOString();
}
...
function particular(row: RestroRdsRow) {
  return [
    sourceLabel(row.EntrySource),
...
export async function GET(req: NextRequest) {
  try {
    ...
    const currentMonth = getIndiaCurrentMonth();

    const fromDate =
      normalizeDate(req.nextUrl.searchParams.get("from")) ||
      currentMonth.from;

    const toDate =
      normalizeDate(req.nextUrl.searchParams.get("to")) ||
      currentMonth.to;
    ...
    if (fromDate > toDate) {
      return NextResponse.json(
        {
          ok: false,
          error: "From Date, To Date se badi nahi ho sakti",
        },
        {
          status: 400,
          headers: noStoreHeaders(),
    ...
    const openingBalance = roundMoney(
      previousRow?.CurrentBal ?? (rows.length ? rows[0].PreviousBal : 0)
    );
    ...
    const ledgerRows = rows.map((row) => {
      ...
      if (source === "order") {
        ...
        debitNoteCount += 1;
        totalDebitNotes += Math.abs(amount);
      } else if (source === "paymentpaid") {
        paymentPaidCount += 1;
        totalPaymentPaid += Math.abs(amount);
    ...
    const netMovement = roundMoney(
      ledgerRows.reduce(
        (total, row) => total + numberValue(row.SettlementAmount),
        0
      )
    );
    ...
    const closingBalance = roundMoney(
      ledgerRows.length
        ? ledgerRows[ledgerRows.length - 1].CurrentBal
        : openingBalance
    );

    const totalDebit = roundMoney(
      ledgerRows.reduce((total, row) => total + numberValue(row.Debit), 0)
    );

    const totalCredit = roundMoney(
      ledgerRows.reduce((total, row) => total + numberValue(row.Credit), 0)
    );
