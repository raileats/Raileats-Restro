  ...
  return (
    <div className="h-[100dvh] w-full max-w-md mx-auto flex flex-col bg-[#f7f9fc] overflow-hidden relative shadow-2xl select-none touch-action-none">
      ...
      <header className="bg-white px-4 py-3 flex items-center justify-between border-b border-gray-100 flex-shrink-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 font-semibold truncate max-w-[160px]">{restro?.RestroName || "Restaurant"}</p>
          </div>
        </div>
      ...
      <div className="bg-white flex-shrink-0 pt-3 pb-3 border-b border-gray-100 z-40 w-full px-4 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight leading-none">Food Menu</h2>
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mt-1">Manage Menu Items</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="h-full flex flex-col items-center justify-center py-12 text-center">
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm flex flex-col items-center w-full">
              ...
              <h3 className="text-base font-black text-gray-800">No Items Added</h3>
          ...
          menuItems.map((item, index) => {
            const isVeg = item.item_category?.toLowerCase() === "veg" || item.item_category?.toLowerCase() === "jain";
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={`w-4 h-4 rounded border flex items-center justify-center text-[8px] font-bold ${isVeg ? 'border-green-600 bg-green-50 text-green-600' : 'border-red-600 bg-red-50 text-red-600'}`}>
                      ●
                    </span>
                  ...
                  <span className={`font-black text-[9px] px-2 py-0.5 rounded-md ${isOn ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-gray-100 text-gray-500'}`}>
                ...
                <div className="grid grid-cols-2 gap-2 bg-gray-50/50 p-2.5 rounded-2xl border border-gray-100 text-[11px] font-bold text-gray-500">
                  ...
                  <div className="flex items-center gap-1.5 justify-end">
                    <span>Tax (GST):</span>
                    <span className="text-gray-700 font-extrabold">{item.gst_percent || 0}%</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-gray-50 pt-2.5 mt-0.5">
                  <div className="flex items-baseline gap-2">
                    <span className="text-[10px] text-gray-400 font-bold">Base: ₹{item.base_price || item.restro_price}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-gray-400 font-bold block leading-none">Selling Price</span>
                    <span className="text-base font-black text-gray-900 mt-0.5 block">
                      ₹{item.selling_price || item.restro_price}
                    </span>
                  </div>
                </div>
      ...
      <nav className="bg-white border-t border-gray-100 h-16 flex items-center justify-around px-2 flex-shrink-0 z-50 shadow-[0_-4px_12px_rgba(0,0,0,0.04)] pb-safe">
        ...
        <button className="flex flex-col items-center justify-center flex-1 h-full text-[#2f54eb]">
          ...
          <span className="text-[10px] font-black mt-1 tracking-tight">Menu</span>
        </button>
