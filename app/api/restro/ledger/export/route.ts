...
export async function GET(req: NextRequest) {
  try {
    ...
    const fromDate =
      normalizeDate(req.nextUrl.searchParams.get("from")) || defaults.from;

    const toDate =
      normalizeDate(req.nextUrl.searchParams.get("to")) || defaults.to;
    ...
    const { data, error } = await supabase
      ...
      .lte(
        "CreatedAt",
        new Date(`${toDate}T23:59:59.999+05:30`).toISOString()
      )
    ...
    csvRows.push([csvCell("RailEats Vendor Ledger")].join(","));
    csvRows.push(
      [
        csvCell("Restaurant"),
        csvCell(
    ...
    csvRows.push(
      [csvCell("Period"), csvCell(`${fromDate} to ${toDate}`)].join(",")
    );
    csvRows.push("");
    ...
    for (const row of rows) {
      const amount = numberValue(row.SettlementAmount);
      const debit = amount < 0 ? Math.abs(amount) : 0;
      const credit = amount > 0 ? amount : 0;

      csvRows.push(
        [
          formatIndiaDateTime(row.CreatedAt),
          ...
          numberValue(row.PreviousBal).toFixed(2),
          numberValue(row.CurrentBal).toFixed(2),
        ]
          .map(csvCell)
          .join(",")
      );
    ...
    console.error("VENDOR LEDGER EXPORT ERROR =>", error);

    return NextResponse.json(
      {
        ok: false,
        error: error?.message || "Unable to export ledger",
      },
      {
        status: 500,
        headers: noStoreHeaders(),
